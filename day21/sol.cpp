...
int main() {
  ...
  unordered_map<string, int> allIng;
  unordered_set<string> allAlg;
  unordered_set<string> remAlg;
  unordered_set<string> okIng;
  unordered_set<string> notOkIng;

  unordered_map<string, unordered_set<string>> algs;
  unordered_set<string> doneAlg;
  unordered_set<string> doneIng;
    ...
    string p1 = trim(sp[0]);
    string p2 = trim(sp[1].substr(9));
    vector<string> ing = split(p1, ' ');
    vector<string> alg = split(p2, ',');
    for (auto& s : alg) s = trim(s);
    for (auto& s : alg) allAlg.insert(s);
    for (auto& in : ing) {
      allIng[in] += 1;
    }
    foods.push_back({
      unordered_set<string>(ing.begin(), ing.end()),
      unordered_set<string>(alg.begin(), alg.end())
    });
    for (auto& al : foods.back().second) {
      for (auto& in : foods.back().first) {
        algs[al].insert(in);
      }
    }
	}
  ...
  while (!remAlg.empty()) {
    vector<string> tdAlg;
    for (auto& al : remAlg) {
      vector<unordered_set<string>> allPoss;
      for (auto& f : foods) {
        if (f.second.count(al)) {
          unordered_set<string> poss;
          for (auto& in : f.first) {
            if (doneIng.count(in)) continue;
            poss.insert(in);
          }
          allPoss.push_back(poss);
        }
      }
      unordered_set<string> res = allPoss[0];
      for (int i = 1; i < allPoss.size(); i++) {
        auto& poss = allPoss[i];
      ...
      if (res.size() == 1) {
        string in = vector<string>(res.begin(), res.end())[0];
        doneIng.insert(in);
        doneAlg.insert(al);
        mp1[in] = al;
        tdAlg.push_back(al);
  ...
  long ans = 0;
  for (auto& in : allIng) {
    if (!doneIng.count(in.first)) {
      ans += allIng[in.first];
    }
  }
  vector<string> res1(doneAlg.begin(), doneAlg.end());
  sort(res1.begin(), res1.end());
  vector<string> res2(doneIng.begin(), doneIng.end());
  unordered_map<string, int> sortIdx;
  for (int i = 0; i < res1.size(); i++) {
    sortIdx[res1[i]] = i;
  }
  sort(res2.begin(), res2.end(), [&](string& a, string& b) {
    return sortIdx[mp1[a]] < sortIdx[mp1[b]];
  });
  for (auto& s : res2) {
    cout << s << ",";
  }
